...
class Sampler:
    ...
    def __init__(self, main_dir=None,CaseStudy=0,seq = 3,ndraw=10000,thin = 1,  nCR = 3, 
                 ...
                 lik_sigma_est=False,parallel_jobs=4,jr_scale=1.0,rng_seed=123,DGM=None,
                 iplimits=[0,0],pt=0,saved_model='',outdir='',r_e=0.03,wwav=-1,c_sigma=1):
        ...
        if self.CaseStudy==4:
            ...
            DNN.device = device
            DNN.cuda = cuda
            Extra.w = torch.Tensor(
                np.genfromtxt(
                    main_dir+'wavelet_simm.asc')[np.newaxis,np.newaxis,:].T).to(device)*wwav
                
            #SGAN generator
            if MCMCPar.DGM=='GAN':
                from GAN import uGenerator_2layers
                # load parameters of trained SGAN
                gpath = 'GAN_generator_epoch1000.pth'
                dnnmodel = uGenerator_2layers(60).to(DNN.device)
                for param in dnnmodel.parameters():
                    param.requires_grad = False
                dnnmodel.to(DNN.device)
                dnnmodel.eval()
                dnnmodel.load_state_dict(torch.load(main_dir+gpath, map_location=device))
                DNN.netG = dnnmodel.forward
                DNN.nz=1
                samp = torch.load(f'Test_models/{saved_model}')[None,:]
                
            # VAE generator 
            elif MCMCPar.DGM=='VAE':  
                from VAE import VAE_2layers as VAE
                gpath = 'VAE_epoch1000.pth'
                dnnmodel = VAE(z_dim=60)
                dnnmodel.load_state_dict(torch.load(main_dir+gpath, map_location=device))
                for param in dnnmodel.parameters():
                    param.requires_grad = False
                dnnmodel.to(DNN.device)
                dnnmodel.eval()
                DNN.netG = dnnmodel.forwardNT
                samp = torch.load(f'Test_models/{saved_model}')[None,:]
                
            if pt==1: 
              _, Measurement.MeasData= DNN.netG(samp.to(device))
              
            # else: 
            #   Measurement.MeasData= torch.load('test_models/'+model_ip).to(DNN.device)
            #   print(pt,model_ip,Measurement.MeasData.shape)
              
            Measurement.MeasData= (Extra.Ip_lmts[1]-Extra.Ip_lmts[0])*Measurement.MeasData.to(device)+Extra.Ip_lmts[0]
            Measurement.MeasData = fullstack_seismic(Measurement.MeasData,Extra.w ).detach().cpu()
            
            Measurement.sigma = r_e*np.abs(Measurement.MeasData)+c_sigma
            Measurement.noise = torch.randn_like(Measurement.MeasData)*Measurement.sigma
            Measurement.MeasData = Measurement.MeasData + Measurement.noise
            
            DNN.dims = (samp.shape[-1],) # !!!! adapted for fullstack invey  bgrsion
            self.ndim=samp.shape[-1]
            MCMCPar.n=self.ndim
            DNN.threshold=False
            self.DNN=DNN
            ...
            Measurement.N=(Measurement.MeasData.shape[-1]*Measurement.MeasData.shape[-2])
            #Measurement.Sigma= 1 # Measurement error is 1 ns
            Measurement.relative_error=r_e
            Measurement.c_sigma= c_sigma
            
            if MCMCPar.Prior == 'StandardNormal':          
                MCMCPar.BoundHandling=None 
        
        elif self.CaseStudy==3:
            
            ModelName='nonlinear_pygimli'
            MCMCPar.lik=2
            MCMCPar.savemodout=False
            MCMCPar.lik_sigma_est=False
            DNN=AttrDict()  
            ...
            DNN.device = device
            DNN.cuda = cuda
            Extra.bh_spacing = 6.5
            Extra.bh_length = 12.5
            Extra.sensor_spacing = 0.5
            ...
            if MCMCPar.DGM=='SGAN':
                ...
                dnnmodel = Generator(cuda=DNN.cuda, gpath=gpath).to(DNN.device)
                for param in dnnmodel.parameters():
                    param.requires_grad = False
                dnnmodel.to(DNN.device)
                dnnmodel.eval()
                ...
                Measurement.MeasData=np.load('mg5_data_noise1.npy')

            # VAE generator
            elif MCMCPar.DGM=='VAE':  
                from autoencoder_in import VAE
                gpath = 'VAE_inMSEeps100r1e3.pth'
                dnnmodel = VAE(cuda=DNN.cuda, gpath=gpath)
                for param in dnnmodel.parameters():
                    param.requires_grad = False
                dnnmodel.to(DNN.device)
                dnnmodel.eval()
                DNN.netG = dnnmodel.decode
                samp = np.load('test_models/zinitVAE500.npy')[0]
                Measurement.MeasData=np.load('mv5_data_noise1.npy')           
            ...
            DNN.netG = Generator(cuda=DNN.cuda, gpath=DNN.gpath, nc = DNN.nc, nz = DNN.nz)
            ...
            # Load measurements
            file_name='truemodel_sig0_5.npy'
            Measurement.MeasData = np.load(file_name)
            Measurement.Sigma=0.5 # Measurement error is 1 ns
            Measurement.MeasData = Measurement.MeasData[0:576]
    ...
    def _init_sampling(self):
        ...
        self.Extra.of = np.empty([MCMCPar.seq,0])
        self.Extra.rof = np.empty([MCMCPar.seq,0])
        self.Extra.log_p = np.empty([MCMCPar.seq,0])
        ...
        if MCMCPar.lik== 3.5: 
            rof, of, log_p = CompLikelihood(X,fx0,self.MCMCPar,self.Measurement,self.Extra)
            
        else: of,log_p = CompLikelihood(X,fx0,self.MCMCPar,self.Measurement,self.Extra)
        ...
        if self.MCMCPar.save_tmp_out==True:
            with open(MCMCPar.outdir+'/'+'out_tmp'+'.pkl','wb') as f:
                 ...
                 'ModelName':self.ModelName,'Extra':self.Extra,'DNN':self.DNN},f, protocol=pickle.HIGHEST_PROTOCOL)
...
#            start_time = time.time()
            ...
            for gen_number in range(0,self.MCMCPar.steps):
                ...
                if MCMCPar.lik== 3.5: 
                    rof_xnew, of_xnew, log_p_xnew = CompLikelihood(xnew,fx_new,self.MCMCPar,self.Measurement,self.Extra)
            
                else: of_xnew,log_p = CompLikelihood(xnew,fx_new,self.MCMCPar,self.Measurement,self.Extra)
                
                self.Extra.of = np.append(self.Extra.of, of_xnew, axis=1)
                self.Extra.rof = np.append(self.Extra.rof,rof_xnew,axis=1)
                self.Extra.log_p = np.append(self.Extra.log_p, log_p_xnew, axis=1)

                # Calculate the Metropolis ratio
                accept = Metrop(self.MCMCPar,xnew,log_p_xnew,xold,log_p_xold,alfa_s,self.Extra)
        ...
        if self.MCMCPar.saveout==True:
            with open(MCMCPar.outdir+'/'+'dreamzs_out'+'.pkl','wb') as f:
                pickle.dump({'Sequences':self.Sequences,'Z':self.Z,'OutDiag':self.OutDiag,'fx':self.fx,'MCMCPar':self.MCMCPar,'Measurement':self.Measurement,'Extra':self.Extra,'DNN':self.DNN},f
